# ÉTAPE 14 — Présent, formes dérivées & situations de vie

> Étapes terminées : 0 → 13E.
> Cette étape : conjugaison présent (مضارع), formes dérivées II-III, et 3 situations de vie (marché, famille, voyage) avec dialogue engine.
...
**Ce qui est OUT de É14 :**
- Conjugaison impératif / futur proche (É15)
- Formes IV → X (Phase 3)
- Vocabulaire thématique exhaustif (É16+)
- Mode quiz chronométré (É15)
- Révision spaced-repetition pour grammaire (É16)

---

## MISSION 0 — Scan initial

Lance `@codebase-scout` puis confirme l'état du repo avant de commencer. Vérifie en particulier : les tables `conjugation_entries`, `words`, `grammar_rules` et le lesson engine existant.
...
- [ ] `@arabic-content-validator` sur les 48 entrées → pas d'erreur
...
**Contexte :** Le fill-in-the-blank est l'exercice naturel de la conjugaison — l'apprenant doit produire la bonne forme verbale dans un contexte de phrase. Le type `'fill_blank'` existe déjà dans l'union `ExerciseType` (depuis É13E), mais le composant dédié et l'interface détaillée n'ont pas encore été implémentés. Cette mission les crée.

### 6a — Types TypeScript

Dans `src/types/exercise.ts`, vérifier que `'fill_blank'` est bien dans l'union `ExerciseType` (il devrait l'être). Puis ajouter l'interface détaillée :
...
// Vérifier que 'fill_blank' est déjà dans l'union ExerciseType.
// Ajouter FillBlankExercise à la discriminated union des configs :
// export type Exercise = ... | FillBlankExercise;
```
...
> **IMPORTANT (post-É12 reskin)** : Ce composant DOIT utiliser `useTheme()` pour toutes les couleurs — PAS d'import `design-tokens` ni de hex en dur. Respecter `ExerciseComponentProps` comme les composants existants (`FlashcardExercise`, `WriteExercise`). Le code ci-dessous est un guide fonctionnel — adapter les styles au design system `useTheme()` actuel.
...
- [ ] `/checkpoint` → tout vert
- [ ] `@arabic-content-validator` sur les seeds de cette mission → pas d'erreur

---

## MISSION 8 — Intégration lesson engine : M7 et M8

**Contexte :** Le lesson engine doit router les leçons de M7 vers `generatePresentTenseFillBlanks` + MCQ conjugaison, et les leçons de M8 vers le dialogue engine enrichi. Le `exerciseRegistry` contient déjà : `mcq`, `match`, `reorder`, `dialogue`, `listen_select`, `flashcard`, `write`. On y ajoute `fill_blank` (Mission 6) et on étend le lesson engine pour M7/M8.
...
### 10a — Validation automatique toolkit

Lance `@regression-tester`. Tous les axes doivent être ✅.

### 10b — Validation contenu arabe

Lance `@arabic-content-validator` sur l'ensemble des seeds É14 (conjugaisons présent, nouveaux verbes, formes II-III, dialogues).

### 10c — Scénarios de test É14 (manuels)
...
| 10 | `@regression-tester` + `@arabic-content-validator` + tests manuels | ⬜ |

---

## GESTION /docs

**Fichiers à conserver dans /docs après É14 :**
- `ETAPE-14-present-situations.md` (ce fichier)
- `lisaan-seed-letters.json`

**Fichiers à supprimer de /docs :**
- `ETAPE-13E-review-engine-polymorphique.md` (terminée)

---

> **Prochaine étape après validation :** Étape 15 — Impératif, futur proche (سَوْفَ / سَـ), mini-jeux chronométrés (speed round, memory match), et premier mode "défi quotidien" avec récompenses spéciales.
